...
The Helm chart installs irc and by default the following dependencies using subcharts:
...
```bash
helm install my-irc  --version 0.1.0
```
...
| `settings.subdomain` | the subdomain of the installation excluding www. | `irc` |
| `settings.subpath` | Any sub path to follow the domain, like /api/v1 | `irc` |
